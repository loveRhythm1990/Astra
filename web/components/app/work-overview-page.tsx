...
export function WorkOverviewPage({
  ...
  const [branchInteractions, setBranchInteractions] =
    useState<WorkBranchInteractionPageV1 | null>(initialInteractions ?? null);
  ...
  useEffect(() => {
    setBranchInteractions(initialInteractions ?? null);
  }, [initialInteractions, workId, selectedBranchId]);
  ...
  const currentBranchInteractions =
    branchInteractions?.work_id === workId && branchInteractions.branch_id === branchId
      ? branchInteractions
      : null;

  const refreshBranchInteractions = useCallback(async () => {
    const generation = eventRefreshGeneration.current;
    try {
      const result = await refreshWorkBranchInteractionsAction({
        workId,
        branchId: selectedBranchId,
      });
      ...
      if (
        ...
        result.page.branch_id !== selectedBranchId
      ) {
        return false;
      }
      setBranchInteractions(result.page);
      return true;
    } catch {
      return false;
    }
  ...
  useVisiblePoll({
    enabled:
      activityNeedsGraphRefresh ||
      turnActive ||
      Boolean(currentBranchInteractions?.interactions.length),
    intervalMs: ACTIVE_WORK_EVENTS_REFRESH_MS,
    maximumIntervalMs: 30_000,
    refresh: refreshBranchInteractions,
  });
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div className="min-w-0 space-y-6">
            ...
            <WorkTurnComposer
              ...
              initialInteractions={currentBranchInteractions}
...
function WorkRecoveryPointsCard({
  ...
  const portableWorkspaceSaved =
    points?.points.some(
      (point) => point.coverage.workspace && point.capabilities.has_portable_workspace,
    ) ?? false;
  const workspaceStateSaved =
    points?.points.some((point) => point.coverage.workspace) ?? false;
  return (
    <Card id="work-progress" className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-text-muted">
            {portableWorkspaceSaved
              ? "Conversation, Work state, and a verified workspace package are recorded at stable points."
              : workspaceStateSaved
                ? "Conversation, Work state, and workspace state are recorded at stable points."
                : "Conversation and Work state are recorded at stable points."}
          </p>
        </div>
      ...
      <p className="text-xs leading-5 text-text-muted">
        {portableWorkspaceSaved
          ...
          : "Conversation restore and workspace migration are not available from these records yet."}
      </p>
    </Card>
  );
}
