    ...
    let config = if let Some(servers) = servers {
        ...
        for value in servers.split(',') {
            ...
            let address: SocketAddr = value.parse().or_else(|_| value.parse::<IpAddr>().map(|ip| SocketAddr::new(ip, 53)))
                .map_err(|_| "ASTRA_BYOK_DNS_SERVERS must contain comma-separated DNS IP addresses, optionally with ports")?;
            ...
            let mut ns = NameServerConfig::udp_and_tcp(address.ip());
...
pub(super) async fn resolve(
    ...
    let lookup = tokio::time::timeout(Duration::from_secs(5), resolver.lookup_ip(fqdn)).await
        ...
        .map_err(|_| "Astra Server could not resolve the model endpoint; check its DNS configuration and the provider hostname")?;
