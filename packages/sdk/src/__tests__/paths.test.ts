...
describe("paths — helpers encode ids", () => {
  ...
  test("sessionTranscriptPath", () => {
    expect(sessionTranscriptPath("s/1")).toBe(
      `/sessions/${encodeURIComponent("s/1")}/transcript`,
    );
  });
  ...
  test("sessionArtifactDownloadPath matches Rust safe segment behavior", () => {
    ...
    expect(sessionArtifactDownloadPath("s1", "a1")).toBe(
  ...
  test("skillPath encodes id", () => {
    expect(skillPath("n@1.0.0")).toBe(
      `/skills/${encodeURIComponent("n@1.0.0")}`,
    );
  });

  test("skillUnpublishPath encodes name", () => {
    expect(skillUnpublishPath("a/b")).toBe(
      `/skills/${encodeURIComponent("a/b")}/unpublish`,
    );
  });

  test("model paths encode model names", () => {
    expect(modelPath("bedrock/claude")).toBe(
