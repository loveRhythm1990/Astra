...
When `MEMORIA_WEB_URL` is unset and a `MEMORIA_MASTER_KEY` is explicitly configured, composition selects the trusted self-hosted authority instead. Password-authenticated Astra users are bound to the master-key transport by their canonical Astra `user_id`, and data requests use Memoria's `Memoria-Owner` authorization scheme. Memoria validates the deployment secret but exposes a non-master principal, so ID-based reads and mutations retain atomic owner enforcement. An older Memoria server that does not support the scheme rejects these requests instead of receiving an unrestricted Bearer master credential. This mode is not an error fallback and must not be activated for hosted/browser-login deployments.
